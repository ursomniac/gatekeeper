...
def can_object_page_be_shown(user, this_object, including_parents=False):
    ...
    """
    try:
        #print "user: ", user, ' is staff: ', user.is_staff, ' is auth: ', user.is_authenticated
        #print "test: ",  this_object.publish_status >= 0 or this_object.treat_as_standalone == 1
        if user.is_staff: # admin users can always see pages
            ...
            #print "\t user is staff but ", this_object.publish_status >= 0, ' or ', this_object.treat_as_standalone == 1
            #else:
...
def get_appropriate_object_from_model(this_model):
    ...
    qs2 = qs.order_by('-date_modified')
    if len(qs2) > 0:
        return qs2[0]
        
    # Send the most-recent "default"
    qs3 = qs.filter(default_live=True).order_by('-date_modified')
    if len(qs3) > 0:
        return qs3[0]
